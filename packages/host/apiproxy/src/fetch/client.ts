...
import { jobCancelValueSchema } from '../api/jobs.schema.ts'
...
export interface IApiClient {
  ...
  jobs: {
    cancel(payload: RequestPayload<'job.cancel'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'job.cancel'>>>
  }
  host: {
...
const UNARY_VALUE_SCHEMAS: { [K in keyof RpcMethodMap]: z.ZodType<Wire<ResponseValue<K>>> } = {
  ...
  'job.cancel': jobCancelValueSchema,
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  readonly jobs: IApiClient['jobs'] = {
    cancel: (payload, signal) => this.callUnary('job.cancel', payload, signal),
  }
