...
Web background-job feature owner: contributes one entry to `conversation.session.header.actions` listing the `ctx.jobs` records this session can see. The data arrives through the `jobsBySession` list mirror that [`dsh-client-runtime`](../runtime/README.md) folds from `session/jobs` frames. A running row exposes a Stop action through the Host's `job.cancel` RPC; the component receives that mutation as a session-bound callback and otherwise holds only popover, pending-action, and failure state.
...
Settled rows stay visible and de-emphasized until the registry drops them at owner disposal. They are in the snapshot, and a failed job's `detail` is the only place its failure is legible. A running one-shot background subagent therefore appears both here and in the [subagent catalog](../ui-subagent/README.md): the catalog navigates into the child's transcript, while this list owns cancellation of its background job.

Only `running` and `stopping` rows carry the action. One click disables the button immediately and keeps the popover open; the `session/jobs` push then advances the row to `stopping` and its terminal state. A transport or Host rejection re-enables the button and renders the returned failure on that row. The Host receives both `sessionId` and `jobId` and rechecks registry ownership, so a guessed foreign id never becomes a cancellation capability.
...
- **Output stays read-only** — no Web path calls `ctx.jobs.read()`, because it would consume the model's only output cursor. Human cancellation calls `kill()`, which marks the task reported and therefore suppresses its automatic completion notice; the model can still observe the terminal state through `job_list` or an explicit `job_output`.
