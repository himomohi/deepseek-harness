...
이 저장소는 [`himomohi/deepseek-harness`](https://github.com/himomohi/deepseek-harness) 포크입니다. 공식 플러그인 아키텍처를 유지하면서 한국어 Web UI, 모바일 폭 레이아웃, 더 안전한 업데이트와 종료 명령, 순서를 보존하는 대용량 스트리밍, 제공자별 출력 한도, 백그라운드 작업 취소, 선택형 브라우저 알림을 관리합니다.
...
npm 명령은 이 포크를 설치하지 않습니다. 한국어 UI, 업데이트 도구, 모바일 수정, 전송 계층 변경이 필요하면 `himomohi/deepseek-harness`를 복제해야 합니다.
...
| 더 안전한 업스트림 업데이트 | `dsh update`가 공식 커밋을 미리 보여 주고 필요하면 얕은 Git 이력을 복구하며 병합 확인, 버전만 다른 `package.json` 통과, 포크 기능 본문 유지, 재빌드, 포크 기능 검증을 수행합니다. |
| 백그라운드 작업 중지 | 세션 헤더 작업 목록에서 Host `job.cancel` 명령으로 실행 중인 작업을 취소할 수 있습니다. |
| 브라우저 알림 | 일반 설정의 선택 항목이 페이지에 포커스가 없을 때 질문과 완료된 응답을 알립니다. |

OpenCodex는 공식 모델 설정의 `llm-pi-ai` 제공자로 연결합니다. 이 포크는 전용 OpenCodex 어댑터를 포함하지 않습니다.
...
| `pnpm dsh update` | 공식 기본 브랜치를 대화형으로 병합하고, 기계적 충돌을 넘긴 뒤 재빌드하고 포크 마커를 검증합니다. |

남은 내용 충돌, 빌드 실패, 구현 마커 누락이 있으면 업데이트를 중단하고 수정 안내를 제공합니다. 부분 업데이트를 성공으로 보고하지 않습니다.
