...
이 저장소는 [`himomohi/deepseek-harness`](https://github.com/himomohi/deepseek-harness) 포크입니다. 공식 플러그인 아키텍처를 유지하면서 한국어 Web UI, 모바일 폭 레이아웃, 더 안전한 업데이트와 종료 명령, 선택형 브라우저 알림을 관리합니다.
...
npm 명령은 이 포크를 설치하지 않습니다. 한국어 UI, 업데이트 도구, 모바일 수정이 필요하면 `himomohi/deepseek-harness`를 복제해야 합니다.
