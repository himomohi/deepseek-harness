...
**Component copy rides the standard `t` seat; deep children take `t` as a plain prop** typed `XxxProps['t']`. The dictionary canon uses `zh satisfies Record<string, string>` as the key source and checks every other shipped dictionary against the same key set.
...
- **ui-trajectory uses the same locale registry as the rest of the client**, including its inspection-specific toolbar terminology.
...
- ui-primitives' defaults remain visible until a consumer passes labels; localized plugins pass their active-locale copy explicitly.
- Pinning e2e to English means non-English defaults are covered mainly by package-level component specs and the settings language-switch scenario; browser e2e does not assert every shipped locale's copy.
